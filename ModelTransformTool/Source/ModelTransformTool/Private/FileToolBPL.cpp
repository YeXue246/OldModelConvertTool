...
bool UFileToolBPL::GetPathType(const FString& InputPath)
{
...
FString UFileToolBPL::ZipFolder(const FString& FolderPath)
{
    ...
    FString StdOut;
    FString StdErr;
    const FString TarArgs = FString::Printf(TEXT("-a -c -f \"%s\" -C \"%s\" ."), *FullZipPath, *FullFolderPath);
    FPlatformProcess::ExecProcess(TEXT("tar.exe"), *TarArgs, &ReturnCode, &StdOut, &StdErr);

    if ((ReturnCode != 0 || !FileManager.FileExists(*FullZipPath)))
    {
        UE_LOG(LogTemp, Warning, TEXT("tar zip failed, fallback to PowerShell. Folder: %s, Err: %s"), *FullFolderPath, *StdErr);
...
FString UFileToolBPL::UnzipToFolder(const FString& ZipFilePath, const FString& TargetRootFolder)
{
    ...
    if (!ClearFolderContents(ExtractFolder))
    {
        UE_LOG(LogTemp, Warning, TEXT("Failed to clear extract folder before unzip: %s"), *ExtractFolder);
    }

    int32 ReturnCode = -1;
    FString StdOut;
    FString StdErr;
    const FString TarArgs = FString::Printf(TEXT("-xf \"%s\" -C \"%s\""), *FullZipPath, *ExtractFolder);
    FPlatformProcess::ExecProcess(TEXT("tar.exe"), *TarArgs, &ReturnCode, &StdOut, &StdErr);

    if (ReturnCode != 0)
    {
        UE_LOG(LogTemp, Warning, TEXT("tar unzip failed, fallback to PowerShell. Zip: %s, Err: %s"), *FullZipPath, *StdErr);
    ...
    FileManager.FindFilesRecursive(ExtractedFiles, *ExtractFolder, TEXT("*"), true, false, false);
    if (ExtractedFiles.IsEmpty())
    {
        UE_LOG(LogTemp, Error, TEXT("Unzip completed but no files were extracted: %s"), *ExtractFolder);
        return TEXT("");
    }
